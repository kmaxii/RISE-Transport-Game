...
public class VasttrafikAPI
{
    ...
    private const string JourneyPlanResultLimit = "5";

    
    
    [ItemCanBeNull]
    public static async Task<JourneyResult> GetJourneyJson(string originGid = "9022014001004001",
        string destinationGid = "9022014001950001")
    {
        try
        {
            Client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", AccessToken);
            string url =
                $"{BaseUrl}/journeys?originGid={originGid}&destinationGid={destinationGid}&limit={JourneyPlanResultLimit}";
